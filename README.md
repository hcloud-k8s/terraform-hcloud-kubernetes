...
- **Perimeter Security:** External access to cluster nodes is restricted using [Hetzner Cloud Firewall](https://docs.hetzner.com/cloud/firewalls/) rules that limit exposed ports and allow incoming traffic only from selected sources.
- **Network Policy:** Internal traffic between workloads is governed by Kubernetes Network Policies enforced through [Cilium CNI](https://docs.cilium.io/en/stable/network/kubernetes/policy/), enabling fine-grained control over cluster communication.
- **Encryption in Transit:** Pod-to-pod traffic is transparently encrypted by Cilium using [WireGuard](https://docs.cilium.io/en/latest/security/network/encryption-wireguard/) by default, while [IPsec](https://docs.cilium.io/en/latest/security/network/encryption-ipsec/) is available as an alternative encryption method.
- **Encryption at Rest:** Talos encrypts the [STATE](https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/storage-and-disk-management/disk-encryption#state-partition) and [EPHEMERAL](https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/storage-and-disk-management/disk-encryption#ephemeral-partition) partitions with [LUKS2](https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/storage-and-disk-management/disk-encryption) by default. Each node uses an individual encryption key derived from its unique `nodeID`.
