...
- **Perimeter Security:**
  External access to cluster nodes is restricted using [Hetzner Cloud Firewall](https://docs.hetzner.com/cloud/firewalls/) rules that limit exposed ports and allow incoming traffic only from selected sources.
- **Network Policy:**
  Internal traffic between workloads is governed by Kubernetes Network Policies enforced through [Cilium CNI](https://docs.cilium.io/en/stable/network/kubernetes/policy/), enabling fine-grained control over cluster communication.
- **Encryption in Transit:**
  Pod-to-pod traffic is transparently encrypted by Cilium using [WireGuard](https://docs.cilium.io/en/latest/security/network/encryption-wireguard/) by default, while [IPsec](https://docs.cilium.io/en/latest/security/network/encryption-ipsec/) is available as an alternative encryption method.
- **Encryption at Rest:**
